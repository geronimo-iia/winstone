...
public class ObjectPool implements Runnable {
    ...
    public void handleRequest(Socket socket, Listener listener) throws IOException, InterruptedException {
        ...
        synchronized (this.requestHandlerSemaphore) {
            ...
            if (unused > 0) {
                ...
                Logger.log(Logger.FULL_DEBUG, Launcher.RESOURCES, "ObjectPool.UsingRHPoolThread", new String[] {
                    "" + this.usedRequestHandlerThreads.size(), "" + this.unusedRequestHandlerThreads.size()
                });
            }
            ...
            // If we are out (and not over our limit), allocate a new one
                ...
                this.usedRequestHandlerThreads.add(rh);
                Logger.log(Logger.FULL_DEBUG, Launcher.RESOURCES, "ObjectPool.NewRHPoolThread", new String[] {
                    "" + this.usedRequestHandlerThreads.size(), "" + this.unusedRequestHandlerThreads.size()
                });
            }
    ...
    public void releaseRequestHandler(RequestHandlerThread rh) {
        synchronized (this.requestHandlerSemaphore) {
            ...
            Logger.log(Logger.FULL_DEBUG, Launcher.RESOURCES, "ObjectPool.ReleasingRHPoolThread", new String[] {
                "" + this.usedRequestHandlerThreads.size(), "" + this.unusedRequestHandlerThreads.size()
            });
        }
    }
    ...
    public WinstoneRequest getRequestFromPool() throws IOException {
        ...
        synchronized (this.requestPoolSemaphore) {
            ...
            // If we are out, allocate a new one
                ...
                this.usedRequestPool.add(req);
                Logger.log(Logger.FULL_DEBUG, Launcher.RESOURCES, "ObjectPool.NewRequestForPool", "" + this.usedRequestPool.size());
            } else
                throw new WinstoneException(Launcher.RESOURCES.getString("ObjectPool.PoolRequestLimitExceeded"));
        }
        return req;
    }
    ...
    public WinstoneResponse getResponseFromPool() throws IOException {
        ...
        synchronized (this.responsePoolSemaphore) {
            ...
            // If we are out, allocate a new one
                ...
                this.usedResponsePool.add(rsp);
                Logger.log(Logger.FULL_DEBUG, Launcher.RESOURCES, "ObjectPool.NewResponseForPool", "" + this.usedResponsePool.size());
            } else
                throw new WinstoneException(Launcher.RESOURCES.getString("ObjectPool.PoolResponseLimitExceeded"));
        }
        return rsp;
    }
