...
/**
 * Implements a JDBC 2.0 pooling datasource. This is meant to act as a wrapper around a JDBC 1.0 driver, just providing the pool management
 * functions. Supports keep alives, and check-connection-before-get options, as well as normal reclaimable pool management options like
 * maxIdle, maxConnections and startConnections. Additionally it supports poll-retry on full, which means the getConnection call will block
 * and retry after a certain period when the pool is maxed out (good for high load conditions). This class was originally drawn from the
 * generator-runtime servlet framework and modified to make it more JDBC-API only compliant.
 * 
...
public class WinstoneDataSource implements DataSource, Runnable {
    ...
    private List<Connection> usedWrappers;
    private List<Connection> unusedRealConnections; // sempahore
    private List<Connection> usedRealConnections;
     ...
     */
    public WinstoneDataSource(String name, Map<String, String> args, ClassLoader loader) {
        this.name = name;
        
        this.usedWrappers = new ArrayList<Connection>();
        this.usedRealConnections = new ArrayList<Connection>();
        this.unusedRealConnections = new ArrayList<Connection>();
    ...
    protected Connection getConnection(int retriesAllowed) throws SQLException {
        ...
        synchronized (this.unusedRealConnections) {
            ...
            if (this.unusedRealConnections.size() > 0) {
                ...
                log(Logger.FULL_DEBUG, "WinstoneDataSource.UsingPooled", new String[] {
            ...
            // If we are out (and not over our limit), allocate a new one
                ...
                log(Logger.FULL_DEBUG, "WinstoneDataSource.UsingNew", new String[] {
                    "" + this.usedRealConnections.size(), "" + this.unusedRealConnections.size()
                }, null);
                try {
                    return prepareWrapper(realConnection);
        ...
        synchronized (this.unusedRealConnections) {
            ...
            if (realConnection != null) {
                if (this.usedRealConnections.contains(realConnection)) {
                    ...
                    log(Logger.FULL_DEBUG, "WinstoneDataSource.Releasing", new String[] {
                        "" + this.usedRealConnections.size(), "" + this.unusedRealConnections.size()
                    }, null);
                } else {
        ...
        while (threadRunning) {
            try {
                ...
                if ((killInactiveCounter == 0) || (keepAliveCounter == 0)) {
                    log(Logger.FULL_DEBUG, "WinstoneDataSource.MaintenanceTime", "" + (System.currentTimeMillis() - startTime), null);
                }
    ...
    protected void executeKeepAliveOnUnused() {
        ...
        List<Connection> dead = new ArrayList<Connection>();
        
        for (Iterator<Connection> i = this.unusedRealConnections.iterator(); i.hasNext();) {
        ...
        for (Iterator<Connection> i = dead.iterator(); i.hasNext();) {
            killConnection(this.unusedRealConnections, i.next());
        }
        
        log(Logger.FULL_DEBUG, "WinstoneDataSource.KeepAliveFinished", "" + this.unusedRealConnections.size(), null);
    }
    ...
    /**
     * This makes a new rw connection. It assumes that the synchronization has taken place in the calling code, so is unsafe for use outside
     * this class.
     */
    protected Connection makeNewRealConnection(List<Connection> pool) throws SQLException {
        if (this.url == null) {
        ...
        log(Logger.FULL_DEBUG, "WinstoneDataSource.AddingNew", new String[] {
            "" + this.usedRealConnections.size(), "" + this.unusedRealConnections.size()
        }, null);
     ...
     */
    protected void killPooledConnections(List<Connection> pool, int maxIdleCount) {
    ...
    private static void killConnection(List<Connection> pool, Connection conn) {
        pool.remove(conn);
        try {
            conn.close();
        } catch (SQLException err) {
    ...
    public String toString() {
        return DS_RESOURCES.getString("WinstoneDataSource.StatusMsg", new String[] {
    ...
    public boolean isWrapperFor(Class<?> iface) throws SQLException {
        ...
        try {
            ...
            result = connection.isWrapperFor(iface);
        } finally {
            if (connection != null) {
                connection.close();
            }
    ...
    public <T> T unwrap(Class<T> iface) throws SQLException {
        ...
        try {
            connection = getConnection();
            result = connection.unwrap(iface);
        } finally {
            if (connection != null) {
                connection.close();
            }
