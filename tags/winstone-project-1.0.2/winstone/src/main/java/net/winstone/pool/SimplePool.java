 ...
 * 
 * @see http://www.ibm.com/developerworks/java/library/j-jtp07233.html for ConcurrentHashMap choice.
 * @see http://www.ibm.com/developerworks/java/library/j-jtp09275.html?ca=dgr-jw22JavaUrbanLegends for choose what put in a pool.
...
public class SimplePool<T> implements Pool<T> {
    ...
    /** true if pool instance is in a closing way. all released resource will be destroyed. */
     ...
     * @param timeout acquire time out in milliseconds (<=0 infinite waiting).
     * @param minIdle minimum number of idle resource (Upper limit of minIdle is equals to capacity).
     * @throws IllegalArgumentException if capacity <= 0 or if factory is null
     */
     ...
     */
    public void clear() {
        List<T> forDiscard = new ArrayList<T>();
        pooled.drainTo(forDiscard);
        for (T resource : forDiscard) {
     ...
     */
    public int drain() {
        int delta = (pooled.size() - this.minIdle);
        int drained = 0;
        if (delta > 0) {
     ...
     * 
     * @param function the specified process.
     */
    public void applyOnIdle(final Function<Void, T> function) {
        Iterator<T> iterator = pooled.iterator();
    ...
    /**
     * Close the pool. All resource released after this call and all objects sitting idle will be destroyed.
     */
    public void close() {
    ...
    /**
     * Return the number of instances currently borrowed from this pool (Shouldn't be called due to performance leak).
     * 
    ...
    /**
     * Return the number of instances currently idle in this pool. This may be considered an approximation of the number of objects that can
     * be borrowed without creating any new instances. Returns a negative value if this information is not available
     * 
